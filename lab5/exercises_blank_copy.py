...
def train_network(train_loader, main_model, optimizer, scheduler, num_epoch, verbose=False):
    ...
    for data, data_label in train_loader:
        ...
        nloss, prec1 = main_model.forward(data, data_label)
        ...
        if verbose:
            print("Epoch {:1.0f}, Batch {:1.0f}, LR {:f} Loss {:f}, Accuracy {:2.3f}%".format(num_epoch, counter, optimizer.param_groups[0]['lr'], (loss/counter).item(), (top1/counter).item()))

        if scheduler[1] == 'iteration': scheduler[0].step()

    if scheduler[1] == 'epoch': scheduler[0].step()

    return ((loss/counter).item(), (top1/counter).item())
